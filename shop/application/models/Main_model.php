...
class Main_model extends CI_Model
{
	...
	public function new_arrivals()
	{
		...
		$to		= toDate(NOW());
		///  ใช้ตอนทดสอบ
		$qs	= $this->db->where('active', 1)->where('show_in_shop', 1)->order_by('date_upd', 'desc')->limit(10)->get('tbl_product');
		/// ใช้จริง
	...
	public function features()
	{
		...
		$this->db->where('show_in_shop', 1)->where('active', 1)->order_by('default_category_id', 'desc')->limit($item);
	...
	public function moreFeatures($offset)
	{
		...
		//--- ใช้ตอนทดสอบ
		$this->db->where('active', 1)->order_by('date_add', 'desc')->limit($item, $offset);
